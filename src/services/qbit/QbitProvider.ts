...
export default class QBitProvider implements IProvider {
  ...
  private constructor() {
    ...
    // On 403: if an API key is set and failed, clear it and let the caller surface
    // the error. If no key, attempt a single silent re-login with in-memory creds
    // (stored by app.ts during the session), then retry once.
    this.axios.interceptors.response.use(
      res => res,
      async err => {
        if (err?.response?.status !== 403 || this._retrying) {
          return Promise.reject(err)
        }
        // If using API key auth, clear key and surface error (no credential retry)
        if (this.axios.defaults.headers.common['Authorization']) {
          this.clearApiKey()
          return Promise.reject(err)
        }
        // Attempt one silent re-login
        this._retrying = true
        try {
          const { useAppStore } = await import('@/stores/app')
          const appStore = useAppStore()
          const relogged = await appStore.silentRelogin()
          if (relogged) {
            // Retry the original request once
            const result = await this.axios.request(err.config)
            return result
          }
        } catch {
          // ignore
  ...
  /**
   * Configure all subsequent requests to use an API key for auth.
   * When set, the SID cookie is not required.
   */
  ...
  /**
   * Remove the API key header, reverting to cookie-based auth.
   */
  clearApiKey() {
