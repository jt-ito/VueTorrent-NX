...
/** Keep-alive interval in ms when the tab is backgrounded */
...
export const useMaindataStore = defineStore('maindata', () => {
  ...
  // ── Feature 3: Background keep-alive ping ──────────────────────────────
  // Fires every 30s when the tab is hidden to keep the qBit session alive.
  // Uses GET /app/version (tiny payload, always authenticated).
  const { pause: pauseKeepAlive, resume: resumeKeepAlive } = useIntervalFn(
    async () => {
      const vtStore = useVueTorrentStore()
      if (!vtStore.keepAliveEnabled) return
      if (document.visibilityState === 'visible') return // maindata handles visible tabs
      try {
        await qbit.getVersion()
      } catch {
        // ignore; maindata will handle re-auth on next visible poll
      }
    },
