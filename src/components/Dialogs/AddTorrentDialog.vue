...
async function submit() {
  ...
  const torrentFiles = files.value
  const torrentUrls = urls.value
  ...
  if (vueTorrentStore.showPredownloadPicker) {
    ...
    const hashOrNull = await addTorrentStore.addTorrentStopped(torrentFiles, torrentUrls, payload)
    if (!hashOrNull) {
    ...
    dialogStore.createDialog(PredownloadFilesDialog, {
      hash: hashOrNull,
      torrentName: form.value.rename ?? '',
      payload,
      isMagnet,
    })
